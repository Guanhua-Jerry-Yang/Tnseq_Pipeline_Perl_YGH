...
sub avge{
	...
	for($i=4;exists $line[$i];$i=$i+$r)
	{
		...
		if ($r == 3 && $j != 0)
		{
			...
			print $out "\t",sprintf("%.5f",(($ele[$i]+$ele[$i+1]+$ele[$i+2])/$j)),"\t",sprintf("%.5f",(stddev($ele[$i],$ele[$i+1],$ele[$i+2])));#bug: treat no insertion as "0"
		}
		elsif($j !=0 )
		{
			print $out "\t",sprintf("%.5f",(($ele[$i]+$ele[$i+1])/$j)),"\t",sprintf("%.5f",(stddev($ele[$i],$ele[$i+1])));
		
		}
		else
		{
			print $out "\t",$line[$i],"--",$line[$i+$r-1],"\tSTDEV";
		}
	}
	print $out "\n";	
}
